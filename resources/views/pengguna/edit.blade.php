        ...
        ">
            ...
            ">
                <h5 class="card-title" style="
                    ...
                    font-size: 1.5rem;
                ">
                    <i class="fas fa-user-edit me-2" style="color: #3B62A4;"></i> Edit Pengguna
                </h5>
            </div>
            ...
            <div class="card-body" style="padding: 2rem;">
                <form method="POST" action="{{ route('pengguna.update', $user->id) }}">
                    ...
                    <div class="mb-4">
                        <label for="roles" class="form-label" style="
                            font-weight: 600;
                            color: #495057;
                            margin-bottom: 0.5rem;
                        ">
                        ...
                        <select class="form-select" id="roles" name="roles[]" multiple required 
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->name }}" {{ $user->hasRole($role->name) ? 'selected' : '' }} style="padding: 8px 12px;">
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                        <small class="form-text text-muted" style="
                            ...
                            display: block;
                        ">
                            <i class="fas fa-info-circle me-1"></i> Gunakan Ctrl (Windows) atau Cmd (Mac) untuk memilih multiple roles
                        </small>
                        @error('roles')
                            <div class="text-danger mt-1" style="font-size: 0.875rem;">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex justify-content-start gap-3 mt-5">
                        ...
                        onmouseout="this.style.backgroundColor='#1E3C72'; this.style.borderColor='#1E3C72'; this.style.boxShadow='none'">
                            <i class="fas fa-check-circle me-2"></i> Update Pengguna
                        </button>
