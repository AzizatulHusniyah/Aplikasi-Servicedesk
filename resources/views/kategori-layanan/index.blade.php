...
<div class="row justify-content-center" style="margin: 0; padding-top: 20px;">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-5" style="padding-bottom: 20px; border-bottom: 1px solid #e0e0e0;">
            <h4 style="font-weight: 700; color: #1E3C72; margin: 0; font-size: 1.75rem;">Data Kategori Layanan</h4>
            <a href="{{ route('kategori-layanan.create') }}" class="btn btn-primary" style="
        ...
        ">
            <div class="card-body" style="padding: 2rem;">
                    ...
                    <table class="table" style="margin-bottom: 0; border-collapse: separate; border-spacing: 0;">
                        ...
                        <tbody>
                            ...
                            <tr class="table-row" style="transition: background-color 0.2s ease;"
                                ...
                                <td style="vertical-align: middle; padding: 1rem 1.5rem; font-weight: 600; color: #1E3C72;">{{ $kl->nama }}</td>
                                <td style="vertical-align: middle; padding: 1rem 1.5rem; color: #495057;">{{ $kl->keterangan ?: '-' }}</td>
                                <td style="vertical-align: middle; padding: 1rem 1.5rem;">
                                    @if($kl->status_aktivasi)
                                ...
                                <td style="vertical-align: middle; white-space: nowrap; padding: 1rem 1.5rem;">
                                    {{-- Tombol Edit --}}
                                    <a href="{{ route('kategori-layanan.edit', $kl->id) }}" class="btn btn-sm" style="
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <form action="{{ route('kategori-layanan.destroy', $kl->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus kategori layanan {{ $kl->nama }}? Aksi ini tidak dapat dibatalkan.')"
                                        title="Hapus">
                ...
                @if(count($kategoriLayanan) === 0)
                    <div class="text-center py-5">
                        <i class="fas fa-layer-group fa-4x mb-3" style="color: #ccc;"></i>
                        <p class="text-muted" style="font-size: 1.1rem;">Belum ada data Kategori Layanan yang terdaftar.</p>
                        <a href="{{ route('kategori-layanan.create') }}" class="btn btn-primary mt-2" style="
                            background-color: #1E3C72;
                            border-color: #1E3C72;
                            border-radius: 8px;
                            font-weight: 600;
                            padding: 0.6rem 1.5rem;
                        ">
                            <i class="fas fa-plus me-2"></i> Tambah Kategori Layanan Pertama
                        </a>
                    </div>
                @endif
            </div>
